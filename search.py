...
def breadthFirstSearch(problem: SearchProblem):
    ...
    while not queue.isEmpty():
        # Pop the current state and path from the queue
        ...
        if state not in visited:
            ...
            # Enqueue all successors to the queue
            for successor, action, cost in problem.getSuccessors(state):
                if successor not in visited:
...
def uniformCostSearch(problem: SearchProblem):
    ...
    # Initialize priority queue with start state and an empty path
    ...
    while not priority_queue.isEmpty():
        ...
        if state not in visited:
            ...
            # For all successors, update the cost and push to the priority queue
            for successor, action, cost in problem.getSuccessors(state):
...
def aStarSearch(problem: SearchProblem, heuristic=nullHeuristic):
    ...
    # Initialize priority queue with start state, empty path, and initial cost
    priority_queue = PriorityQueue()
    ...
    while not priority_queue.isEmpty():
        ...
        if state not in visited:
            ...
            # For all successors, calculate total cost (cost + heuristic)
            for successor, action, cost in problem.getSuccessors(state):
                new_cost = problem.getCostOfActions(path + [action])
